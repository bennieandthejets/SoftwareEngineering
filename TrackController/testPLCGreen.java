...
public class testPLCGreen implements PLC {
	int startblock;
	int endblock;

	
	public HashSet<Integer> switches = new HashSet<>(Arrays.asList(12, 29, 58, 62, 76, 86));
	...
	private HashMap<String, Integer> findNextSwitch(Train train) {
		...
		for (int routeStep = 0; routeStep < route.length; routeStep++) {
			...
			if (routeStep == 0 && train.position == 152) {
				HashMap<String,Integer> switchInfo = new HashMap<String,Integer>();
				switchInfo.put("switchOn",152);
		...
		if (firstSwitch != -1 && train.position == 152) {
			HashMap<String,Integer> switchInfo = new HashMap<String,Integer>();
			if (switchIndex == 0) {
